<script lang="ts">
	...
	import type { Cell } from '$lib/_snakeGame';
	import type { Connection, Organism } from "@snaike/neat";
	import { Population, Node, NodeType} from '@snaike/neat';
	
	import OrganismVis from '$lib/components/OrganismVis.svelte';
	...
	const TICK_TIME = 1;
	const SIZE = 10;
	let representative: Organism | null;
	let grid: Cell[][];
	let gamesInPlay = 0;
...
{#if representative}
	<OrganismVis organism={representative}></OrganismVis>
{/if}
</div>