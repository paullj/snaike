...
class Genome {
  ...
  getConnectionAt(innovation: number): Connection {
    return this._connections.get(innovation);
  }
  ...
  mutate(config: Config): void {
    const {
      ...
      mutateSetEnableProbability: reEnableGeneProbability,
    ...
    if (Math.random() < mutateAddNodeProbability) {
      ...
      if (Math.random() < reEnableGeneProbability) {
        this.mutateSetConnection(true);
      }
    }
  }

  mutateModifyActivation(node: Node = null): void {
  ...
  mutateAddConnection(config: Config): void {
    const { connectionStrength: mutationPower, maxAddConnectionTries: addConnectionTries } = {
      ...config
    };

    let maxTries = addConnectionTries;
    ...
    while (maxTries--) {
      ...
      const connection = new Connection(from, to, randomBetween(-mutationPower, mutationPower));
      const isValid =
  ...
  mutateConnectionsWeights(config: Config): void {
    const {
      connectionStrength: mutationPower,
      connectionPertubationProbability: genomeWeightPerturbated
    } = { ...config };

    this.connections
      .filter(({ enabled }) => enabled)
      .forEach((connection) => {
        const random = randomBetween(-mutationPower, mutationPower);
        const perturbed = Math.random() < genomeWeightPerturbated;
